...
use Facades\App\Data\Services\AuthService;

use Facades\App\Data\Services\FrameworkService;
use Facades\App\Data\Services\ProgramService;
use Facades\App\Data\Services\SubControlService;use Illuminate\Support\Collection;use Illuminate\Support\Facades\DB;
use Mail;
...
class MarketplaceService
{
    ...
    public function getMarketplaceFilters($filters = [], $programId = null)
    {
        ...
        $providers = Company::active()->whereIn('id', $providersIds)->orderBy('name', 'ASC')->get();
    ...
    public function getMarketPlaceQuery($programId)
    {
        $apps = DB::select(DB::raw("select
            ...
            join framework_app_template_map framework_map on framework_map.framework_id = p.framework_id and framework_map.app_template_id = app.app_template_id
    ...
    public function getMarketplaceResults($filters = [], $programId = null, $isAppend = true)
    {
        ...
        $solutionsQuery = Solution::with(['type', 'company', 'categories', 'subCategories', 'ppt'])
            ...
            ->join('company', function ($join) use ($marketplaceCategoryIds) {
                $join->on('solution.company_id', '=', 'company.id')
    ...
    public function getCompanyChart($filters = [], $programId = null)
    {
        $chartData     = [];
        $subcontrolTmp = [];
        ...
        $results = $this->getMarketplaceResults($filters, $programId);
        ...
        if (empty($filtered)) {
            array_sort_by_column($chartData, 'no_of_subcontrol', SORT_DESC);
    ...
    public function saveSolution($request, $userId)
    {
        ...
        if (!empty($request) && $request['status'] == 'contacted') {
    ...
    public function activityTrigger($slug, $data)
    {
        if ($slug) {
            $this->captureLog($slug, $data);
            return true;
        }
        return false;
    }
